...
class Rabbitmq:
    ...
    def declare_local_queue(self, routing_key, queue_name=""):
        # Creates a local queue.
        # Rabbitmq server will clean it if the connection drops.
        result = self.channel.queue_declare(queue=queue_name, exclusive=True, auto_delete=True)
        self._l.debug(f"Declared queue result is {result}.")
    ...
    def subscribe(self, routing_key, on_message_callback, queue_name=""):
        created_queue_name = self.declare_local_queue(routing_key, queue_name)
