...
#define MODEL_IDENTIFIER TestBench
#define MODEL_GUID "{31db0ec3-addd-4a38-b54b-0ac8aabf3699}"
...
extern void TestBench_setupDataStruc(DATA *data, threadData_t *threadData);
#define fmu2_model_interface_setupDataStruc TestBench_setupDataStruc
...
void setDefaultStartValues(ModelInstance *comp) {
  ...
  comp->fmuData->modelData->realParameterData[4].attribute.start = 1.0;
  ...
  comp->fmuData->modelData->realParameterData[8].attribute.start = 1.0;
...
static const int realAliasIndexes[29] = {
  17, 0, 2, 20, 2, 2, 17, 17, 0, 16, 16, 20, 21, 24, 15, 3, 2, 3, 2, -25,
  2, 16, 0, -17, -17, 2, 24, 2, -17
};
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 32 && (comp->state == model_state_instantiated || comp->state == model_state_initialization_mode)) {
